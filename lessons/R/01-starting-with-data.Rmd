...
If we want to know the average weight of all the animals, 
...
And then take we can take the mean of the 'wgt' column (column 8) to get mean weight

First let's look at that column
```{r}
datamatrix[,8]
```

We see it's what we want so we can calculate the mean

```{r}
mean(datamatrix[,2])
```

Oops, we get an error. That's because in that column 
...
```{r}
animal_1 <- dat[1, ] # first row, all of the columns
paste('wgt for animal 1:', max(patient_1))
```
