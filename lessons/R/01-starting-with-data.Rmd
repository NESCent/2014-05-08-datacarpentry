...
## Statistics for multiple samples
...
aggregate(data_you_want_to_calculate, factor_to_sort_on, function)

```{r}
tapply(dat2$wgt, dat2$species, mean)
```

Now we have all the means and we can even plot it. First let's put this data in a vector
...
```{r}
barplot(species_means, cex.names=0.4)
```

and change the color

```{r}
barplot(species_means, cex.names=0.4, col=c("blue"))
```
...
1. Calculate the average weight by plot
2. Plot the average weight by plot and make the bars red
